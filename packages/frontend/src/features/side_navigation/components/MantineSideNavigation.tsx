...
import {
	MantineSideNavigationItem,
	type SideNavigationItemProps,
} from "./MantineSideNavigationItem";
 ...
 */
export function MantineSideNavigation({ isCompact }: { isCompact: boolean }) {
			...
			<Stack h="full">
				<Stack w="100%" px={10} pt={8} gap={0}>
					{sideNavigationItems.map((item) => (
						<MantineSideNavigationItem key={item.name} {...item} />
					))}
				</Stack>
				{isCompact ? null : (
					<>
